...
@attributes { struct symbol *symbols; } pars pars_rec;
...
@attributes { struct ast_node *node; struct symbol *symbols; } args expr mul_terms or_terms plus_terms term unary_term
...
| term '^' TK_ASSIGNMENT expr ';' stats     @{ @codegen generate_write(@term.node@, @expr.node@, sym_needed_registers(@stats.0.symbols@));
                                               ...
                                               @i @stats.1.depth@ = @stats.0.depth@;
                                               @i @stats.1.symbols@ = @stats.0.symbols@; @}
| term ';' stats                            @{ @codegen generate_term(@term.node@, sym_needed_registers(@stats.0.symbols@));
                                               @i @term.symbols@ = @stats.0.symbols@;
                                               @i @stats.1.depth@ = @stats.0.depth@;
                                               @i @stats.1.symbols@ = @stats.0.symbols@; @}
;
...
args
:               @{ @i @args.node@ = ast_guard(); @}
| expr          @{ @i @args.node@ = ast_operator(OP_ARG, ast_guard(), @expr.node@);
                   @i @expr.symbols@ = @args.symbols@; @}
| expr ',' args @{ @i @args.0.node@ = ast_operator(OP_ARG, @args.1.node@, @expr.node@);
                   @i @expr.symbols@ = @args.0.symbols@;
...
| TK_ID '(' args ')' @{ @i @term.node@ = ast_operator(OP_CALL, @args.node@, ast_string(@TK_ID.string@));
