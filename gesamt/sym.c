...
sym_add_var(struct symbol *symbols, const char *string)
{
    ...
    if (symbols != NULL) {
        if (symbols->reg == REG_END) {
            fputs("No more free registers for variables\n", stderr);
            exit(4);
        }
...
sym_lookup_label(struct symbol *symbols, const char *name)
{
    ...
    fprintf(stderr, "the label '%s' doesn't exist in the scope\n", name);
    exit(3);
}
...
sym_lookup_var(struct symbol *symbols, const char *name)
{
    ...
    fprintf(stderr, "the variable '%s' doesn't exist in the scope\n", name);
    exit(3);
}
